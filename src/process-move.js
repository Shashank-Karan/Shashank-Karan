...
const { commandFromIssue, parseMove } = require('./issue');
const { loadConfig, loadGame, loadPlayers, writeJson } = require('./state');
const { addCapture, recordResult, updatePlayer } = require('./stats');
...
function applyIssue(root, issue) {
  ...
  const command = commandFromIssue(issue);
  if (command === 'resign') {
    game.finished = true;
    game.players[game.turn] = game.players[game.turn] || player;
    updatePlayer(players, player, game.turn, game.history.length + 1);
    players.players[player].resignations += 1;
    ...
    game.players[game.turn] = game.players[game.turn] || player;
    updatePlayer(players, player, game.turn, game.history.length + 1);
    players.players[player].passes += 1;
    ...
    if (game.passes >= 2) {
      game.finished = true;
      game.score = scoreGame(game, config.komi);
      game.result = { type: 'score', ...game.score };
    }
  } else {
