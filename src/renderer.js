...
function renderBoard(game, repository = 'OWNER/REPOSITORY') {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" role="img" aria-label="${size} by ${size} Go board" viewBox="0 0 ${width} ${width}" width="${width}" height="${width}"><style>svg{max-width:100%;height:auto;background:#d8a24a;border:1px solid #633f1e}.grid{stroke:#633f1e;stroke-width:1}.stone{stroke:#321f0e;stroke-width:1}.legal{fill:#b8782e;opacity:.28}.black{fill:#151515}.white{fill:#f8f4e8}.last{fill:#e84c3d;stroke:#fff;stroke-width:1}text{font:12px sans-serif;fill:#432916}</style><g class="grid">${lines.join('')}</g><g>${stones.join('')}</g><g>${labels.join('')}</g></svg>`;
}
